...
def _get_spark():
    ...
    try:
        ...
        s = (
            ...
            .appName("TokenOptimizer-Databricks")
...
def run_training(spark, sample_mode=False, cv_folds=3):
    ...
    def build_pipe(reg, target):
        ...
        return Pipeline(stages=[asm, scl, reg.setLabelCol(target).setFeaturesCol("features")])
    ...
    logger.info(f"   CV R²={cv_metrics['r2']:.4f}  bestRegParam={cv_metrics['best_regParam']}")
    ...
    logger.info(f"🏆 Best: {best_name}  R²={best_r2:.4f}  time={summary['total_training_time_s']:.1f}s")
    return summary
...
def run_cost_analysis(spark, sample_mode=False, pricing_model="gpt-4o"):
    ...
    logger.info(f"  {pricing_model}: raw=${t.get('total_cost_raw_usd',0):.4f}  "
...
def main():
    ...
    if not args.skip_preprocess:
        try:
            r = run_preprocessing(spark, sample_size=args.sample_size if args.sample else None)
    ...
    if not args.skip_cost:
        try:
            r = run_cost_analysis(spark, sample_mode=args.sample,
